...
fun JoyStick(
    ...
    size: Dp = 170.dp,
    dotSize: Dp = 40.dp,
    backgroundImage: Int = R.drawable.joystick_background,
    dotImage: Int = R.drawable.joystick_dot,
    moved: (x: Double, y: Double) -> Unit = { _, _ -> }
) {
    Box(
        ...
        Image(
            ...
            "JoyStickBackground",
            modifier = Modifier.size(size).graphicsLayer(alpha = 0.5F),
        )
        ...
        Image(
            ...
            modifier = Modifier
                ...
                .onGloballyPositioned { coordinates ->
                    var width = coordinates.boundsInParent().width
                    var height = coordinates.boundsInParent().height
                    moved(
                        ((coordinates.positionInParent().x - centerX) / (width+centerX/2)).toDouble(),
                        (-(coordinates.positionInParent().y - centerY) / (height+centerY/2)).toDouble()
                    )
                }
        )
    }
